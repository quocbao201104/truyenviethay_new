  ...
  actions: {
    async fetchUserProfile() {
      ...
      try {
        ...
        // Thêm timestamp vào avatar URL để tránh cache trình duyệt khi load profile lần đầu
    ...
    async updateUserProfile(data: UpdateUserPayload) {
      ...
      try {
        const response = await updateMeApi(data); // Lấy response từ API

        if (response.user) {
            // Thêm timestamp vào avatar URL để tránh cache trình duyệt khi cập nhật
            if (response.user.avatar) {
                response.user.avatar = response.user.avatar + '?' + Date.now();
            }
            this.profile = response.user;
            ...
            // Nếu backend không trả về user mới (trường hợp không có avatar mới được gửi)
            // hoặc bạn muốn đảm bảo dữ liệu mới nhất, hãy gọi lại fetchUserProfile
