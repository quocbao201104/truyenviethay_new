...
export async function updateMeApi(data: UpdateUserPayload): Promise<any> {
  const formData = new FormData(); // Dùng FormData vì có thể có file upload (avatar)
  ...
  if (data.avatar instanceof File) {
    formData.append('avatar', data.avatar); // Thêm file nếu là File
  } else if (data.avatar === null) {
    formData.append('avatar', ''); // Gửi chuỗi rỗng nếu muốn xóa avatar
  }

  // Axios sẽ tự động đặt Content-Type là 'multipart/form-data' khi dùng FormData
 ...
 * @returns Promise<any> Phản hồi từ server.
 */
export async function changePasswordApi(data: ChangePasswordPayload): Promise<any> {
