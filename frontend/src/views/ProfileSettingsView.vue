    ...
    <main class="settings-container">
      ...
      <section v-else class="settings-card">
        ...
        <form @submit.prevent="handleSubmit" class="settings-form">
          <div class="form-group avatar-group">
            ...
            <div class="avatar-preview">
              ...
              <input
                ...
                ref="avatarInput"
              />
              <button type="button" class="avatar-btn" @click="avatarInput?.click()">
          ...
          <button type="submit" class="submit-btn" :disabled="userStore.isUpdatingProfile">
...
// SỬA DÒNG NÀY: Import ảnh theo cách của Vite (đảm bảo file tồn tại ở đường dẫn này)
...
const originalForm = ref<Omit<typeof form.value, 'avatar' | 'current_password' | 'new_password' | 'confirm_new_password' | 'agree'>>({
  full_name: '',
  email: '',
  phone: '',
  gender: 'other',
...
const avatarInput = ref<HTMLInputElement | null>(null); // Khai báo ref cho input file
...
watch(() => userStore.profile, (newProfile) => {
  if (newProfile) {
    ...
    // SỬA DÒNG NÀY: Chỉ sao chép các trường cơ bản
    originalForm.value = {
...
const avatarUrl = computed(() => {
  ...
  if (userAvatar && userAvatar !== '/uploads_img/avatar/default-avatar.jpg') {
    // Đảm bảo URL đầy đủ đến backend (nếu avatar được lưu trên backend)
    return `http://localhost:3000${userAvatar}`; 
  }
  return defaultAvatar; // Sử dụng defaultAvatar đã import
});

const handleAvatarError = (event: Event) => {
  const target = event.target as HTMLImageElement;
  target.src = defaultAvatar; // Sử dụng defaultAvatar đã import
};
...
const handleSubmit = async () => {
  ...
  try {
    if (hasProfileChanged.value) {
      ...
      // Chỉ thêm vào payload nếu giá trị thực sự thay đổi hoặc là avatar
      ...
      if (form.value.gender !== originalForm.value.gender) updatePayload.gender = form.value.gender;
      
      // Xử lý avatar:
      if (form.value.avatar) { // Nếu có file mới được chọn
        updatePayload.avatar = form.value.avatar;
      } else if (avatarPreview.value === null && userStore.getUserProfile?.avatar && userStore.getUserProfile.avatar !== '/uploads_img/avatar/default-avatar.jpg') {
        // Nếu avatarPreview là null (người dùng đã xóa ảnh cũ bằng cách click vào "Chọn ảnh" nhưng không chọn file nào)
      ...
      // Chỉ gọi updateUserProfile nếu có các trường cần cập nhật (không tính mật khẩu)
