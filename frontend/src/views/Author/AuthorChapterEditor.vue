...
const handleSubmit = async () => {
    ...
    try {
        const payload = {
            truyen_id: storyId,
            ten_chuong: form.value.ten_chuong,
            so_chuong: form.value.so_chuong, // Backend helper should handle auto-increment if null
...
onMounted(async () => {
    if (isEditMode) {
        ...
        try {
            ...
            const found = chapterStore.chapterList.find(c => c.id === chapterId);
            if (found) {
                form.value.ten_chuong = found.ten_chuong;
                form.value.so_chuong = found.so_chuong;
                form.value.noi_dung = found.noi_dung;
            } else {
                 showErrorToast("Không tìm thấy thông tin chương locally.");
            }
        } catch (e) {
             console.error(e);
        } finally {
