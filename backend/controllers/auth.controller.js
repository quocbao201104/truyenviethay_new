...
exports.updateMe = async (req, res) => {
  ...
  if (req.file) {
    // Trường hợp 1: Có file mới được Multer xử lý thành công
    avatarPathToDB = "/uploads_img/avatar/" + req.file.filename;
    console.log("updateMe Controller: New avatar path from upload:", avatarPathToDB);
  } else if (req.body.hasOwnProperty('avatar') && (req.body.avatar === '' || req.body.avatar === 'null')) {
    // Trường hợp 2: Frontend yêu cầu xóa avatar (gửi trường 'avatar' với giá trị rỗng hoặc 'null')
    avatarPathToDB = null; // Gán null để lưu vào database
    console.log("updateMe Controller: Request to set avatar to NULL.");
  } else {
    // Trường hợp 3: Không có file mới và không có yêu cầu xóa.
    // avatarPathToDB vẫn là undefined, nghĩa là không cập nhật trường avatar trong DB.
    console.log("updateMe Controller: No avatar file and no explicit clear request. Avatar will not be updated.");
  }
  ...
  try {
    ...
    if (req.file || (req.body.hasOwnProperty('avatar') && (req.body.avatar === '' || req.body.avatar === 'null'))) {
        updateData.avatar = avatarPathToDB; // Đây có thể là đường dẫn mới hoặc null
        console.log("updateMe Controller: Avatar field added to updateData:", updateData.avatar);
    } else {
        console.log("updateMe Controller: Avatar field NOT added to updateData (no change).");
    }

    console.log("updateMe Controller: Final updateData sent to model:", updateData);
    ...
    console.log("updateMe Controller: User.updateUser affected rows:", affectedRows);

    if (affectedRows === 0) {
      return res.status(200).json({ message: "Cập nhật thành công nhưng không có thay đổi nào được ghi nhận trong DB." });
    }
...
exports.changePassword = async (req, res) => {
  ...
  console.log("changePassword Controller: old_password received:", old_password ? "******" : "empty");
  console.log("changePassword Controller: new_password received:", new_password ? "******" : "empty");
  ...
  try {
    ...
    const user = results[0];
    console.log("changePassword Controller: User found. User password (partial):", user.password ? user.password.substring(0, 10) + '...' : 'N/A');
    ...
    if (isNewPasswordSameAsOld) {
        console.log("changePassword Controller: New password is same as old password (after hash compare).");
    ...
    console.log("changePassword Controller: updatePassword affected rows:", updatedAffectedRows);

    if (updatedAffectedRows === 0) {
      return res.status(400).json({ message: "Không thể cập nhật mật khẩu. Có thể mật khẩu mới giống mật khẩu cũ." });
    }

    res.json({ message: "Đổi mật khẩu thành công!" });
  } catch (err) {
    console.error("Lỗi server khi đổi mật khẩu trong changePassword Controller:", err);
    res.status(500).json({
