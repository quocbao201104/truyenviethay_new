...
const createHistory = async (data) => {
  try {
    ...
    const oldLevel = await UserLevel.getById(data.old_level_id); // Cấp bậc cũ
    const newLevel = await UserLevel.getById(data.level_id); // Cấp bậc mới
    ...
    const timeSpent = data.timeSpent || 0; // Ví dụ: 30 ngày đã tiêu hao
    ...
    const start_date = new Date(); // Ngày thăng cấp, ví dụ: ngày hôm nay
    const end_date = new Date(start_date);
    end_date.setDate(start_date.getDate() + remainingLifespan); // Cộng số ngày còn lại vào start_date

    // Lưu lịch sử cấp bậc vào DB
    const historyData = {
      user_id: data.user_id,
      level_id: data.level_id,
      start_date: start_date, // Ngày bắt đầu
